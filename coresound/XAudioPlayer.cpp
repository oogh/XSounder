...
void XAudioPlayer::refreshQueueBuffer() {
...
ALenum XAudioPlayer::checkError() {
...
bool XAudioPlayer::ConfigOpenAL(ALuint channels, ALuint bits, ALuint frequency) {
    ALenum format;
    if (8 == bits) {
        if (1 == channels) {
...
void XAudioPlayer::setLoop(ALint isLoop) {
    alSourcei(mSourceId, AL_LOOPING, isLoop);
}


void XAudioPlayer::setVolume(ALfloat volume) {
    alSourcef(mSourceId, AL_GAIN, volume);
}


void XAudioPlayer::setSpeed(ALfloat speed) {
    alSpeedOfSound(speed);
}


ALint XAudioPlayer::NumOfBuffProcessed() {
    ALint bufNum;
...
ALSourceState XAudioPlayer::SourceState() {
    ALint val;
    ALSourceState state;
...
void XAudioPlayer::OpenAudio(ALubyte *buffer, ALuint length) {
...
void XAudioPlayer::PauseSound() {
    if (alSource_paused != SourceState()) {
        alSourcePause(mSourceId);
    }
}

void XAudioPlayer::StopSound() {
    if (alSource_stopped != SourceState()) {
        alSourceStop(mSourceId);
    }
}

void XAudioPlayer::close() {
